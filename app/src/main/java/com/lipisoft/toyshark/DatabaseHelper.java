...
public class DatabaseHelper extends SQLiteOpenHelper {
    private static final String TAG = "ToyShark.Database";

    private static final String DB_NAME = "ToyShark";
    ...
    private final static int MSG_HTTP = 9;
    private final static int MSG_SENSITIVE = 10;
    ...
    private void createTableHTTP(SQLiteDatabase db)
    {
        Log.i(TAG, "Creating HTTP table");
        db.execSQL("CREATE TABLE http ("+
                "ID INTEGER PRIMARY KEY AUTOINCREMENT" +
                ", host TEXT" +
                ", connection TEXT" +
                ", upgrade TEXT" +
                ...
                ", cookie TEXT" +
                ");");
        db.execSQL("CREATE UNIQUE INDEX idx_http ON http(host, referer)");
    }

    private void createTableSensitive(SQLiteDatabase db)
    {
        Log.i(TAG, "Creating Table of sensitive data");
        db.execSQL("CREATE TABLE sensitive ("+
                "ID INTEGER PRIMARY KEY AUTOINCREMENT" +
                ", sensitive_text TEXT" +
                ");");
        db.execSQL("CREATE UNIQUE INDEX idx_sensitive ON sensitive(sensitive_text)");
    }
    ...
    public Cursor getHTTP(String host, String referer)
    {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = getReadableDatabase();
            String queryString = "SELECT * FROM http WHERE host = ? AND referer = ?";
            return db.rawQuery(queryString, new String[]{host, referer});
        } finally {
            lock.readLock().unlock();
        }
    }

    public Cursor getSensitiveText(String sensitive_text)
    {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = getReadableDatabase();
            String queryString = "SELECT * FROM sensitive WHERE sensitive_text = ?";
            return db.rawQuery(queryString, new String[]{sensitive_text});
        } finally {
            lock.readLock().unlock();
        }
    }
    ...
    public boolean httpExists(String host, String referer)
    {
        lock.readLock().lock();
        boolean exists = false;
        try{
            SQLiteDatabase db = getReadableDatabase();
            String queryString = "SELECT COUNT(host) as existing FROM http WHERE host = ? AND referer = " + referer;
            Cursor c = db.rawQuery(queryString, new String[]{host, referer});
            if(!c.moveToFirst())
                exists = false;
            else
            {
    ...
    public boolean sensitiveExists(String sensitive_data)
    {
        lock.readLock().lock();
        boolean exists = false;
        try{
            SQLiteDatabase db = getReadableDatabase();
            String queryString = "SELECT COUNT(sensitive_text) as existing FROM sensitive WHERE sensitive_text = ?";
            Cursor c = db.rawQuery(queryString, new String[]{sensitive_data});
            if(!c.moveToFirst())
                exists = false;
            else
            {
